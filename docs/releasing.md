...
The publisher record and automated publish access are operational. A dedicated Microsoft Entra managed identity is federated to the GitHub `marketplace-production` environment, is a Contributor to publisher `stefanpieter`, and successfully published `stefanpieter.hermes-ai-agent-maintained@3.6.0` without a PAT or client secret. The Keychain item above records the human owner identity only.
